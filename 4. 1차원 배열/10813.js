...
let realArr = tempArr.map((num,idx) => {
  return idx + 1
})

for (let i = 0; i < M; i++){
  let [a, b] = inputData[i + 1].split(' ').map(Number)
  let front = realArr[a - 1] 
  let back = realArr[b - 1]
...
for (let i = 0; i < M; i++) {
  ...
  let temp = realArr[a - 1];

  realArr[a - 1] = realArr[b - 1];
  realArr[b - 1] = temp;
}
